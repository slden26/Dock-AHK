...
class DockPanel {
    ...
    Render() {
        ...
        ; Массив для хранения HWND иконок для расчетов
        this.IconList := []
        
        Loop Files, this.Path "\*.lnk" {
            try {
                FileGetShortcut(A_LoopFileFullPath, &Target)
                xPos := (A_Index=1) ? "x25 y10" : "x+15 y10"
                pic := this.Gui.Add("Picture", xPos " w" this.IcoSize " h" this.IcoSize, "HICON:" LoadPicture(Target, "w" this.IcoSize " h" this.IcoSize " Icon1", &Type))
    ...
    OnDrop(files) {
        ...
        ; Список текущих файлов без префиксов для чистой сортировки
        currentFiles := []
        Loop Files, this.Path "\*.lnk" {
        ...
        relX := mX - dX - 25 ; 25 - начальный отступ
        step := this.IcoSize + 15
        insertIdx := Floor(relX / step) + 1
        if (insertIdx < 1) 
            insertIdx := 1
        if (insertIdx > currentFiles.Length + 1) 
            insertIdx := currentFiles.Length + 1
        ...
        ; Переименовываем всё с новыми индексами для сохранения порядка
